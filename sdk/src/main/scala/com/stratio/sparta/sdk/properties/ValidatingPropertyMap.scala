  ...
  def getBoolean(key: K): Boolean =
      ...
      case Some(value) =>
        Try(value.toString.toBoolean) match {
          case Success(v) => v
          case Failure(ex) => throw new IllegalStateException(s"$value is not parsable as boolean", ex)
        }
      case None => throw new IllegalStateException(s"$key is mandatory")
    }
    ...
    m.get(key) match {
      case Some(value: String) =>
        Try(value.toInt) match {
          case Success(v) => v
          case Failure(ex) => throw new IllegalStateException(s"$value is not parsable as Int", ex)
        }
